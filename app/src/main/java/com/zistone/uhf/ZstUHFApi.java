...
public class ZstUHFApi extends SerialPortManager{
	...
	private byte CMD_SET_SELECT_COMAND_TAG = (byte)0x0C;//设置Select参数
	private byte CMD_SET_SELECT_MODE_TAG = (byte)0x12;//设置Select模式
	...
	public ZstUHFApi(Context context, ZstCallBackListen callback){
		mContext = context;
		mCallback = callback;
	}
	...
	public boolean sendData(byte head, byte command, int len, byte[] data){
	...
	public boolean writeCradTag(byte[] password, byte membank, int addr, int tag_len, byte[] tag_data){
		...
		for(int j=0; j < tag_data.length; j++){
			Log.d(TAG,"j = " + j);
			Log.d(TAG,"tag_data[i] = " + tag_data[j]);
			data[i+j] = tag_data[j];
		}
		return sendData(CMD_HEAD_BB, CMD_WRITE_CARD_TAG, len, data);
	}
	
	/*
	 * 设置Select参数
	 * @param 	setSelectCommend 
	 * 			Ptr
	 * 			Truncate
	 * 			Mask
	 * @return true 成功<br/>
	 * 		   false 失败
	 * */
	public boolean setSelectCommend(int SelParam, byte[] Ptr, int Truncate, byte[] Mask){
		...
		return sendData(CMD_HEAD_BB, CMD_SET_SELECT_COMAND_TAG, len, data);
	}
	
	/*
	 * 设置Select模式
	 * @param 	mode 0x00 在对标签的所有操作之前都预先发送Select指令选取特定的标签。
	 * 				 0x01在对标签操作之前不发送Select指令。
	 * 				 0x02仅对除轮询Inventory之外的标签操作之前发送Select指令，如在
	 *				 Read，Write，Lock，Kill之前先通过Select选取特定的标签。
	 * @return true 成功<br/>
	 * 		   false 失败
	 * */
	public boolean setSelectMode(int mode){
		int len = 1;
		byte[] data = new byte[len];
		data[0] = (byte) mode;
		return sendData(CMD_HEAD_BB, CMD_SET_SELECT_MODE_TAG, len, data);
	}
}